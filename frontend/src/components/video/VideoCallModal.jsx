...
const VideoCallModal = ({
  ...
  if (callType === "voice") {
    ...
    return (
      <div className="fixed inset-x-4 top-4 md:inset-x-auto md:right-4 md:w-96 bg-[#111111] border border-[#222222] rounded-2xl shadow-2xl p-4 z-50 flex flex-col items-center gap-4 animate-slide-in">
        <div className="flex items-center gap-3 w-full">
          {avatar ? (
            <img src={avatar} alt="" className="w-12 h-12 rounded-full object-cover border border-[#222222]" />
          ) : (
          ...
          <div className="flex-1 text-left min-w-0">
            <h4 className="text-sm font-semibold text-[#FFFFFF] truncate">{name}</h4>
        ...
        {isInCall && (
          <div className="flex items-end justify-center gap-1 h-8 w-full py-1">
            {[...Array(6)].map((_, i) => (
              <span
                key={i}
                className="w-1 bg-[#FFD166] rounded-full animate-wave"
                style={{
                  height: "20%",
  ...
  return (
    <div className="fixed inset-0 bg-black z-50 flex flex-col justify-between items-center text-white">
          ...
          <div className="flex flex-col items-center gap-4">
            {avatar ? (
              <img
                src={avatar}
                alt=""
                className="w-24 h-24 rounded-full object-cover border-4 border-[#222222] animate-pulse"
              />
            ) : (
              <div className="w-24 h-24 rounded-full bg-[#FF6B00] text-[#FFFFFF] flex items-center justify-center font-bold text-3xl animate-pulse">
                {name.charAt(0).toUpperCase()}
              </div>
            )}
      ...
      <div className="z-10 w-full p-6 bg-gradient-to-b from-black/80 to-transparent flex flex-col items-center">
        <h2 className="text-lg font-bold text-[#FFFFFF]">{name}</h2>
        {isInCall && (
          <p className="text-xs text-[#FFD166] font-mono tracking-wider">
